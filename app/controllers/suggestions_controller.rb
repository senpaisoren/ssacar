class SuggestionsController < ApplicationController
  ...
  def index
    ...
    if which_texts == nil
      @texts = Text.where("texttype = 'Suggestion'").sort_by {|obj| ((((obj.get_upvotes.size - obj.get_downvotes.size)+1) * 10000000000000000000) / (Time.now.to_i - obj.created_at.to_i)) }.reverse
    end
    if which_texts == 'top'
      @texts = Text.where("texttype = 'Suggestion'").sort_by {|obj| ((obj.get_upvotes.size - obj.get_downvotes.size)+1)}.reverse
    end
    if which_texts == 'hot'
      @texts = Text.where("texttype = 'Suggestion'").sort_by {|obj| ((((obj.get_upvotes.size - obj.get_downvotes.size)+1) * 10000000000000000000) / (Time.now.to_i - obj.created_at.to_i)) }.reverse
    end
    if which_texts == 'new'
      @texts = Text.where("texttype = 'Suggestion'").sort_by {|obj| (Time.now.to_i - obj.created_at.to_i)}
    end
    if texts_search != nil
      @texts = Text.where("title LIKE ? OR content LIKE ? AND texttype IS ?", "%#{texts_search}%", "%#{texts_search}%", "Suggestion")
    end
  end
